...
**Status**: ✅ Complete
**Started**: 2026-05-27
**Plan File**: `.agents/plans/daily-liquidity-refresh-redesign.md`

### Reports Generated

**Execution Report:** `.agents/execution-reports/daily-liquidity-refresh-redesign.md`
- 11/11 tasks completed across 4 parallel waves; ~20 new tests; 984/1003 passing; 0 regressions (+15 net passing vs baseline)
- Three divergences: `compute_live_hl_mid` used instead of raw `_session_bars` for initial seeding (better); 3 supplementary edge-case tests deferred (non-blocking); pre-existing test failures in dispatch_order fixed as collateral improvement
- All acceptance criteria met; `overnight_range` field removal silently disables chop guard (accepted interim state per plan NOTES)
...
- Three divergences: `continue`â†’`abort_merge` flag fix for correct merge-skip semantics; duration math drops -1 offset (dedup handles overlap); test expectation updated to match correct abort behavior
- Live IB probe (step 6) skipped â€” requires live IB Gateway
- Alignment score: 9/10

---

## Feature: D7 â€” Session-End Close + IB Disconnect Hard Close
### Planning Phase
**Status**: âœ… Planned
**Started**: 2026-05-20
...
- No divergences from plan; all acceptance criteria met
- Live PMT end-to-end smoke test pending â€” requires live PMT credentials (intentionally out of scope)
- Alignment score: 10/10
...
Pre-session Databento rolling-window parquet backfill (`data/databento_backfill.py`) added and called at orchestrator startup. `_gap_fill()` call removed from `IbRealtimeSource.start()`. IB Gateway disconnect detection via `ib.disconnectedEvent` â†’ `IbGatewayDisconnectedError` â†’ subprocess exit code 2 â†’ orchestrator closes position and exits code 3. 24 new tests added; 1042 passed / 4 pre-existing failures / 1 skipped (live IB integration) in full suite.
...
- Manual IB integration tests (Tests 1â€“4) pending â€” require live IB Gateway hardware
- Alignment score: 9/10
...
Extracted the shared dailyâ†’trendâ†’hypothesisâ†’strategy dispatch sequence into a new `session_pipeline.py` (`SessionPipeline` class). Fixed all 8 live/backtest behavioral divergences (ATH seeding, hist_1hr/4hr to hypothesis, run_strategy every 1m bar, bar_dict body fields, unified hourly resample, today_bars â‰¤09:20 to run_daily, all-day `recent` scope, all-day MNQ/MES to run_hypothesis). Three callers refactored: `backtest_smt.run_backtest_v2`, `signal_smt.SmtV2Dispatcher`, `automation/main.SmtV2Dispatcher`.
...
**Execution Report (Task 4 â€” V2 Live Dispatcher Wiring):** `.agents/execution-reports/session-pipeline-v2-dispatcher-wiring.md`
...
- Manual live-order smoke test (Manual Test 1) pending â€” requires live PMT credentials and Tradovate demo account
- Alignment score: 10/10
...
- Manual tests (live Databento API + live IB Gateway) pending â€” environment constraint, not a code gap
- Alignment score: 9/10
...
- Manual demo validation (Level 0 + Level 4) pending â€” requires live PMT credentials
- Alignment score: 9/10


## Feature: SMT Redesign â€” JSON-File Architecture + Module Decomposition
...
- E2E smoke: `run_backtest_v2('2025-11-14','2025-11-14')` â†’ 4 trades, PnL âˆ’$471.00; `regression.py` exits 0
- Alignment score: 9/10
...
- Three divergences: 51 tests vs ~46 planned (extra MOVE_LIMIT + cancel+replace tests â€” good); original_limit_price semantics clarified in test assertion; SYMMETRIC_SMT_ENABLED=False added to _patch_base for test isolation
- Backtest smoke: mean_test_pnl=$5,023.74 (within Â±10% of baseline)
- Alignment score: 9/10
...
Adds Equal Highs / Equal Lows detection to the DOL candidate pool used by `_build_draws_and_select`. Computes EQH/EQL levels once per session from prior-day + overnight MNQ bars (swing point clustering with tolerance + staleness invalidation), stores them on `SessionContext`, and feeds them into `_build_draws_and_select` so the existing `secondary_target` mechanism can pick them when they are the closest qualifying liquidity. Purely additive â€” does NOT change TDO as primary TP, does NOT modify stop/partial/trail mechanics (all locked from prior campaign). Expected lift: +10â€“20% on total P&L concentrated in the 43 secondary-hit trades. Critical risk: staleness logic must correctly deactivate levels that have been closed through (not just wicked through); adversarial unit tests for staleness must precede integration.
...
## Feature: Uncap Hold Time â€” Trail Width + Session Extension

**Status**: âœ… Complete
**Plan File**: `.agents/plans/uncap_hold_time.md`

Widens `TRAIL_AFTER_TP_PTS` from 1.0 to 50.0 pts with two safeguards: a never-widen rule (`stop = max(current, trail)` for longs) and a delayed activation gate (`TRAIL_ACTIVATION_R` â€” trail only fires after price travels R multiples of initial stop past TDO). Extends `SESSION_END` to `"15:15"`. Disables partial exit when trail is active (logically contradictory). Adds `initial_stop_pts` to signal dict. Optimizer grid: 4 Ã— 5 = 20 cells (`TRAIL_AFTER_TP_PTS` Ã— `TRAIL_ACTIVATION_R`).
...
Replaces market-order entry (bar close) with a limit at `anchor_close Â± buffer`. Two modes: same-bar fill (`LIMIT_EXPIRY_SECONDS=None`) and forward-looking limit with new `WAITING_FOR_LIMIT_FILL` state machine state. Bar resolution auto-detected from timestamps â€” works at 1m (backtest) and 1s (live). Adds `anchor_close_price`, `limit_fill_bars`, `missed_move_pts` diagnostic fields and `limit_expired` exit type to `trades.tsv` so missed big-mover trades are fully visible. Both constants default to `None` (zero behaviour change). Optimizer search space: `LIMIT_ENTRY_BUFFER_PTS âˆˆ [None, 0.0, 0.25, 0.5, 1.0]`, `LIMIT_EXPIRY_SECONDS âˆˆ [None, 60.0, 120.0, 300.0]`.
...
## Feature: SMT Solutions Aâ€“E â€” Signal Quality & State Machine Hardening
...
- One plan divergence: Wave 3 signal_smt.py state machine not verbatim-mirrored (inapplicable â€” `_process_scanning` is stateless; Solution C inherited via `_build_signal_from_bar`)
- Test results: 77 passed (unit), 27 passed (integration), 623 full suite; 8 pre-existing failures unchanged
- Alignment score: 9/10

---

## Feature: SMT Solution F â€” ICT-Aligned Draw on Liquidity Target Selection

**Status**: âœ… Complete
**Plan File**: `.agents/plans/smt-solution-f-draw-on-liquidity.md`
**Prerequisite**: `smt-solutions-a-e.md` complete. Isolated cycle â€” validate independently before merging with Aâ€“E optimisation results.

Replaces TDO-only TP with a 6-level draw-on-liquidity hierarchy (FVG â†’ TDO â†’ Midnight Open â†’ Session High/Low â†’ Overnight H/L â†’ PDH/PDL). Signals with no draw satisfying `MIN_RR_FOR_TARGET` (1.5Ã—) and `MIN_TARGET_PTS` (15 pts) are skipped. Adds secondary target and `exit_secondary` exit type. Supersedes `OVERNIGHT_RANGE_AS_TP` and `MIDNIGHT_OPEN_AS_TP` flags.
...
- `exit_secondary` fired on 7 test trades â€” two-level exit path confirmed end-to-end

**Post-review fixes (committed with plan):**
- `backtest_smt.py:439` â€” `_run_ses_high` init changed from `0.0` to `-float("inf")` (symmetric with `_run_ses_low`)
- `strategy_smt.py:1432` â€” added clarifying comment on standalone `tp_breached` setter
- `tests/test_smt_backtest.py` â€” fixed `test_signal_with_pdh_draw_placed` assertion (now checks `tp_name`); added `test_exit_secondary_same_bar_primary_and_secondary_crossed`
- `backtest_smt.py` + `signal_smt.py` â€” added directional guards to `overnight_high/low`, `pdh`, `pdl` draws (consistent with other draw entries)
...
## Feature: SMT Strategy â€” Position Architecture Expansion (Plan 2 of 2)
...
- `HIDDEN_SMT_ENABLED = True` (approved Round 1 â€” +30.6% PnL, now default)
...
1-fold fast backtest (2026-01-19 â†’ 2026-04-12). Full data and verdicts in `.agents/experiment-log.md`.

| Run | Config | final_pnl | win_rate | avg_rr | max_dd | Verdict |
|-----|--------|-----------|----------|--------|--------|---------|
| Baseline | â€” | $5,075 | 57.8% | 7.25 | $108 | â€” |
...
| C-1â€“C-4 | TWO_LAYER+FVG (all configs) | $1,258â€“$2,530 | 57.8% | 6.7â€“7.1 | $30â€“$54 | NEUTRAL |
| D-1 | SMT_FILL_ENABLED | $5,075 | 57.8% | 7.25 | $109 | NEUTRAL |

- **APPROVED (1):** `PARTIAL_EXIT_ENABLED=True, PARTIAL_EXIT_FRACTION=0.33` â€” âˆ’12% max_drawdown ($108â†’$95), âˆ’5.5% P&L ($5,075â†’$4,798). Best risk-adjusted result. Now set as default.
- **REJECTED (3):** `SMT_OPTIONAL` all thresholds â€” displacement entries steal same-session SMT slots, reducing wick SMT count. Needs structural fixes (Plan 3: `DISPLACEMENT_STOP_MODE` + `MIN_HYPOTHESIS_SCORE_FOR_DISPLACEMENT`) before re-test.
- **NEUTRAL (4):** `TWO_LAYER+FVG` all configs â€” `layer_b_triggers=0` in this 60-day momentum window. FVG retracements are regime-dependent. Plan 3 adds `fvg_detected` diagnostic field and `FVG_LAYER_B_REQUIRES_HYPOTHESIS` gate for re-test.
- **NEUTRAL (1):** `SMT_FILL_ENABLED` â€” `fill_entries=0`, P&L identical to baseline ($5,075). No fill divergences formed in this 60-day momentum regime. Re-test in Round 3.

**New effective baseline entering Plan 3:** trades=45, pnl=$4,798, wr=62.2%, avg_rr=6.56, max_dd=$95

---

## Feature: SMT Strategy â€” Reference Level Fix + Signal Quality (Plan 1 of 2)

**Status**: âœ… Complete + Experiments Done
...
1-fold fast backtest (2026-01-19 â†’ 2026-04-12). Full data in `plan1-results.tsv`, full analysis in `.agents/experiment-log.md`.

- **APPROVED (1):** `HIDDEN_SMT_ENABLED=True` â€” +30.6% PnL (+$1,189), lower drawdown, +9 extra trades. Now set as default.
- **REJECTED (2):** `STRUCTURAL_STOP_MODE` â€” kills RR (7.47â†’2.08), structurally incompatible with trailing-stop exits.
- **REJECTED (2):** `INVALIDATION_MSS_EXIT` / `INVALIDATION_SMT_EXIT` â€” fires too close to entry; need `MIN_FAVORABLE_MOVE_PTS` guard before retesting.
- **NEUTRAL (5):** `MIDNIGHT_OPEN_AS_TP`, `SILVER_BULLET_WINDOW_ONLY`, `OVERNIGHT_SWEEP_REQUIRED`, `OVERNIGHT_RANGE_AS_TP`, `INVALIDATION_CISD_EXIT` â€” no effect in this 60-day window; retained for future regime testing.
...
1. **Per-rule hypothesis logging** â€” expose each ICT rule output (`pd_range_case`, `week_zone`, `day_zone`, `trend_direction`, `hypothesis_score`) as per-trade fields in `trades.tsv`, enabling rule-level decomposition.
2. **`HYPOTHESIS_FILTER` flag** in `backtest_smt.py` â€” when True, only aligned signals are taken; enables Outcome B walk-forward validation.
3. **`analyze_hypothesis.py`** â€” CLI script that reads `trades.tsv` and prints aligned vs misaligned performance split with per-group win rate, avg P&L, avg RR, and per-fold consistency check.
4. **Round 2 deferred feature fixes** â€” opt-in constants that unblock rejected/neutral features:
   - `DISPLACEMENT_STOP_MODE: bool = False` â€” correct stop for displacement entries (bar extreme, not SMT structural stop)
   - `MIN_HYPOTHESIS_SCORE_FOR_DISPLACEMENT: int = 0` â€” quality gate on displacement entries via hypothesis score
   - `PARTIAL_EXIT_LEVEL_RATIO: float = 0.5` â€” configurable partial exit target (0=entry, 1=TP); enables ratio tuning in Round 3
   - `fvg_detected: bool` â€” per-trade TSV field; diagnoses whether FVG zones form but don't retrace vs. don't form at all
   - `FVG_LAYER_B_REQUIRES_HYPOTHESIS: bool = False` â€” gates Layer B to hypothesis-confirmed reversal sessions
...
- `hypothesis_smt.py` â€” add `compute_hypothesis_context()` + `_count_aligned_rules()` helper
- `backtest_smt.py` â€” integrate context call, attach rule fields to trade records, add `HYPOTHESIS_FILTER` gate, add displacement stop/score-gate/FVG-gate logic
- `strategy_smt.py` â€” add 4 new constants; update `manage_position()` for `PARTIAL_EXIT_LEVEL_RATIO`
- `analyze_hypothesis.py` â€” new script (mirrors `analyze_gaps.py` pattern)
- `tests/test_hypothesis_analysis.py` â€” new test file (30 tests)

### Round 3 Experiment Results

1-fold fast backtest (2026-01-19 â†’ 2026-04-12). Full analysis in `.agents/experiment-log.md`.

| Run | Config | Trades | PnL | WR | avg_rr | max_dd | Verdict |
|-----|--------|--------|-----|----|--------|--------|---------|
| A-1 | HYPOTHESIS_FILTER=True | 19 | $4,124 | 84.2% | 4.40 | $65 | NEUTRAL â€” fewer trades, similar WR, loses $1.5K; hypothesis doesn't add signal value |
| B-1 | SMT_OPTIONAL=True + MIN_DISPLACEMENT_PTS=10 + DISPLACEMENT_STOP_MODE=True | 47 | $7,148 | 76.6% | 5.52 | $120 | APPROVE â€” +$2K, +7 displacement trades, structurally correct stop |
| C-1 | PARTIAL_EXIT_LEVEL_RATIO=0.33 | 53 | $5,939 | 81.1% | 5.34 | $65 | APPROVE â€” tighter partial exit, same WR, lower drawdown |
| C-2 | PARTIAL_EXIT_LEVEL_RATIO=0.25 | 53 | $5,462 | 81.1% | 4.65 | $65 | REJECT â€” further tightening degrades RR with no WR benefit |
| D-1 | FVG_LAYER_B_REQUIRES_HYPOTHESIS=True + scoreâ‰¥2 | 35 | $3,843 | 80.0% | 5.05 | $125 | DEFERRED â€” layer_b_triggers=0; gating has no effect this regime |
...
- Live smoke test (real MNQ parquets) deferred â€” requires IB session data on disk
...
- Level 6 (smoke test with real data) deferred â€” requires IB-Gateway

---

## Feature: signal_smt.py â€” Tick-Based 1s Bar Ingestion
...
- Detailed implementation summary
- Divergences and resolutions (none â€” 10/10 alignment)
...
(Sep 24, 2025 â†’ today), producing only ~30 test trades across 2 active folds.
Walk-forward optimization on 30 trades overfits badly â€” a minimum of ~100 test
...
- Symbols: `MNQ.c.0` and `MES.c.0` (continuous front-month â€” Databento handles roll stitching)
- Schema: `ohlcv-1m` (no native 5m; resample to 5m in code)
- Date range: `2024-01-01` â†’ today (~2 years, ~120+ expected trades)
- Estimated cost: ~$4.93 one-time for Jan 2024 â†’ Mar 2026 (75.6 MB); trivial
...
1. `data/historical/{ticker}.parquet` â€” Databento download (permanent)
2. `~/.cache/autoresearch/futures_data/5m/{ticker}.parquet` â€” IB cache (ephemeral)
...
- `fetch(ticker, start, end, interval, ...)` â€” calls Databento `client.timeseries.get_range()` with dataset `GLBX.MDP3`, schema `ohlcv-1m`, symbols `[ticker]` (e.g. `"MNQ.c.0"`)
- Resample 1m â†’ 5m using `df.resample("5min").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"})` after download
...
- IB fetch (existing path) remains as fallback or can be removed in favour of Databento-only â€” decision for planner
...
**Tests**
- Unit test: `DatabentSource.fetch()` with mocked `databento.client` â€” verify correct dataset, schema, symbol, and resampling logic
...
- API call pattern:
  ...
  df = data.to_df()
  ```
- Column mapping from Databento: `open`, `high`, `low`, `close`, `volume` â†’ rename to `Open`, `High`, `Low`, `Close`, `Volume`
- Timezone: Databento returns UTC timestamps â€” convert to `America/New_York`
...
- `uv run python train_smt.py` runs with 6 walk-forward folds, each test fold having 15â€“25 trades, total ~120 test trades
...
Research completed 2026-04-01. MNQM6/MESM6 (conIds `770561201`/`770561194`) accept explicit `endDateTime` with good data from Sep 24, 2025 â†’ ~6.5 months, ~37 expected trades. Full implementation spec and test cases in plan file.
...
## Feature: SMT Strategy â€” 5m Optimization Harness + Real-Time Screener Architecture
...
~14 days of 1m history per request (`endDateTime=''` restriction â€” error 10339 for explicit dates),
...
- IB ContFuture + `endDateTime=''` supports `durationStr='3 M'` for 5m bars â†’ ~3 months of data per download
- 3 months of 5m data gives ~40â€“60 kill-zone sessions â†’ enough trades for walk-forward stats
...
- Does NOT use the historical parquet cache â€” operates on streaming bars only

### Tasks for next agent

**Task A â€” Update optimizer to 5m bars**
1. `prepare_futures.py`: change `INTERVAL = "1m"` â†’ `"5m"`, update `BACKTEST_START` to today âˆ’ 90 days
2. `data/sources.py`: verify `_IB_CONTFUTURE_MAX_DAYS` is not applied to 5m (it shouldn't be â€” 5m uses the standard `chunk_days` path and `durationStr='90 D'` should not timeout). If needed, add a `_IB_CONTFUTURE_MAX_DAYS_5M` constant or make the cap interval-aware.
...
**Task B â€” Plan real-time SMT screener** (separate feature, after Task A)
...
### Key constraints
- `_IB_CONTFUTURE_MAX_DAYS = 14` cap in `data/sources.py` is a 1m-specific limit; 5m requests use `_IB_CHUNK_DAYS["5m"] = 60` and are not affected by this constant (the cap is only applied in the `contfuture` branch â€” verify this during implementation)
- The `_compute_fold_params` short-window guard (< 130 bdays â†’ 1 fold) will no longer trigger once 3 months of 5m data are available â€” the full 6-fold walk-forward will run normally
- `MIN_BARS_BEFORE_SIGNAL = 5` was calibrated for 1m bars; for 5m bars each bar is 5Ã— wider so the value may need recalibration (5 Ã— 5m = 25 minutes before signal â€” reasonable for the kill zone)
...
- Divergences and resolutions
- Test results and metrics
- Wave 3 (live IB-Gateway) deferred â€” pending user action
...
### What was completed
- Task 1.1: `data/sources.py` â€” ContFuture support; IB 1m data uses ContFuture + `endDateTime=''`
...
- Task 1.2: `prepare_futures.py` â€” downloads MNQ/MES 1m bars; dynamic dates (7-day window)
  - Live test âœ… â€” 6900 bars each for MNQ and MES downloaded successfully
- Task 2.1: `train_smt.py` strategy functions + constants
  ...
  - `_compute_fold_params` auto-detects short windows (< 130 bdays â†’ 1 fold, minimal test days)
- Task 2.2: `tests/test_smt_strategy.py` â€” 24 unit tests, all passing
- Task 3.1: `train_smt.py` harness â€” run_backtest, _compute_metrics, fold loop
- Task 4.1: `tests/test_smt_backtest.py` â€” 10 integration tests, all passing
...
- Live end-to-end âœ… â€” `uv run train_smt.py` ran full backtest on live IB data
...
that 89% of trades in the 1â€“5d bucket are collectively losing âˆ’$120.97 while 40 trades
held 6+ days generate all positive P&L. Config: 6 folds Ã— 60 days, 14-day holdout,
...
### Core Changes
- `train.py` `screen_day()` â€” added SMA200 computation, two-path check (bull/recovery), `signal_path` variable, relaxed `slope_floor` (0.990 recovery vs 0.995 bull), relaxed RSI range (40â€“65 recovery vs 50â€“75 bull), `signal_path` in return dict
- `screener.py` â€” added `signal_path` to per-ticker row dict, `PATH` column in output tables, `death_cross` added to `_RULES`, `_rejection_reason()` returns `"death_cross"` when SMA50 <= SMA200
- `screener_prepare.py` â€” `HISTORY_DAYS` increased from 180 to 300 for SMA200 support
- `tests/test_screener.py` â€” `make_recovery_signal_df` fixture + 7 recovery unit tests
- `tests/test_screener_script.py` â€” `_write_recovery_parquet` helper + `test_screener_finds_candidate_in_bearish_period` integration test
- `tests/test_e2e.py` â€” RSI string anchors updated for 2 agent-loop mutation tests (unplanned fix required by RSI tuple refactor)

### Test Status
- Automated: âœ… 289 passed, 1 skipped, 1 pre-existing failure (`test_select_strategy_real_claude_code`)
- New tests: 8 (7 unit in test_screener.py, 1 integration in test_screener_script.py)
- All 8 plan-specified test scenarios: âœ… pass
...
### Core Changes
- `train.py` `find_pivot_lows()` â€” vectorized with numpy sliding_window_view (eliminates Python/iloc loop)
- `train.py` `zone_touch_count()` â€” vectorized with numpy boolean array ops
- `train.py` `nearest_resistance_atr()` â€” vectorized with numpy sliding_window_view
- `train.py` `manage_position()` â€” tail-sliced calc_atr14 input (df.iloc[-30:] instead of full df)
- `train.py` `detect_regime()` â€” df.loc[:today] (binary search) + tail-slice mean (iloc[-50:])
- `tests/test_optimization.py` GOLDEN_HASH â€” updated to match new detect_regime code
- `screener.py` â€” rejection diagnostics: `_rejection_reason()` helper + `--diagnose` flag support
- `program.md` â€” experiment loop step 2 updated with python-performance-optimization skill instruction
...
### Core Changes
- `screener_prepare.py` â€” added `MAX_WORKERS` (env: `SCREENER_PREPARE_WORKERS`, default 10), module-level `_process_one(ticker, history_start) -> tuple` helper, replaced sequential `for` loop with `ThreadPoolExecutor` + `as_completed`.
- `prepare.py` â€” added `MAX_WORKERS` (env: `PREPARE_WORKERS`, default 10), replaced sequential `for ticker in TICKERS` loop with `executor.map(process_ticker, TICKERS)`.

### Test Status
- Automated: âœ… 282 passed, 1 pre-existing failure (`test_selector.py::test_select_strategy_real_claude_code`), 0 new failures
...
## Feature: Phase 1 â€” Pre-Market Signal CLI
...
### Core Changes
- `train.py` `screen_day()` â€” added `current_price: float | None = None` param; `price_1030am` uses injected price when provided; `rsi14` and `res_atr` added to return dict.
- `screener_prepare.py` â€” builds/refreshes `SCREENER_CACHE_DIR`; `fetch_screener_universe()` (S&P 500 + Russell 1000 + fallback), `is_ticker_current()`, `download_and_cache()` (incremental: fetches only from last cached date forward, merges and deduplicates).
- `screener.py` â€” pre-market BUY signal scanner; staleness check, gap filter (`GAP_THRESHOLD = -0.03`), armed table sorted by `prev_vol_ratio` desc.
- `position_monitor.py` â€” RAISE-STOP scanner; reads `portfolio.json`, calls `manage_position()`, prints signals where new_stop > current stop.
- `analyze_gaps.py` â€” gap vs PnL analysis from `trades.tsv`; exports `load_trades()`, `compute_gaps()`, `print_analysis()`.
- `portfolio.json` â€” user-maintained template with one example position.

### Test Status
- Automated: âœ… 273 passed, 1 skipped (pre-existing), 0 failed (full suite)
...
- Level 4 (live pre-market run): not executed â€” requires network + live pre-market hours
...
**Status**: âœ… Complete
**Completed**: 2026-03-25
**Plan**: system_upgrade_phases.md â€” "Volume criterion redesign" subsection

### Core Validation
`screen_day()` volume filter replaced with two prior-data-only rules that work correctly pre-market: Rule 3a (`vol_trend_ratio >= 1.0`: 5-day avg >= MA30) and Rule 3b (`prev_vol_ratio >= 0.8`: yesterday >= 0.8Ã— MA30). `today_vol` removed from entry logic entirely. `prev_vol_ratio` and `vol_trend_ratio` added to return dict replacing `vol_ratio`. Tests updated to use `vol_trend_ratio < 1.0` as the mutation target. `strategy_selector.py` fixed to handle `train_pnl = None` in strategy METADATA.

### Test Status
- Automated: âœ… 238 passed, 1 skipped, 0 failed (full suite)
- Pre-existing skip: `test_most_recent_train_commit_modified_only_editable_section` â€” git artefact from ecbc2d2 (Phase 0.1 harness rename)
...
### Core Changes
- `tests/conftest.py` â€” session-scoped `test_parquet_fixtures` fixture downloads AAPL/MSFT/NVDA via yfinance, caches to `~/.cache/autoresearch/test_fixtures/`. Dates: history `2024-04-01` (yfinance 1h 730-day limit), backtest `2024-09-01..2025-11-01`.
- `train.py` mutable â€” `_compute_fold_params()` added; auto-detects short windows (< 130 bdays â†’ 1 fold).
- `train.py` `__main__` â€” fold loop uses `_effective_n_folds` / `_effective_fold_test_days` from `_compute_fold_params`.
- `tests/test_fold_auto_detect.py` â€” 13 unit tests for fold logic and ticker split constraints.
- `tests/test_e2e.py` â€” all 9 integration tests wired to `test_parquet_fixtures`; subprocess tests inject `AUTORESEARCH_CACHE_DIR`.
- `tests/test_optimization.py` â€” 3 `@_live` tests (previously skipped without 389-ticker cache) converted to `@pytest.mark.integration` using the small fixture.
- GOLDEN_HASH updated to `efea3141a0df8870e77df15f987fdf61f89745225fcb7d6f54cff9c790779732`.

### Test Status
- 13/13 `test_fold_auto_detect.py` â€” âœ… passed
- 9/9 `test_e2e.py -m integration` â€” âœ… passed
- 3/3 `test_optimization.py` integration tests â€” âœ… passed
- Full suite: 222 passed, 1 pre-existing failure (`test_most_recent_train_commit_modified_only_editable_section` â€” git history artefact from commit ecbc2d2, predates this feature), 0 new failures

---

## Feature: P0 â€” Price Fix and Trade Instrumentation
...
P0-D: `r_multiple` added to all trade records: `(exit âˆ’ entry) / (entry âˆ’ initial_stop)`.
...
- TICKERS expanded from 85 â†’ 389 (copied from `global-mar24` worktree): all 10 GICS sectors + high-vol ETFs
- Fresh parquet cache downloaded for all 389 tickers (381/389 succeeded; 8 failed: `DFS`, `FI`, `SQ`, `SKX`, `K`, `PARA`, `HES`, `MRO` â€” yfinance delisted/symbol conflicts)
- Cache uses correct `price_1030am` (9:30 bar Close) throughout

### Test Status
- Automated: âœ… 210 passed, 0 failed, 0 skipped (all 12 previously-cache-gated tests now run)
...
### Notes
- `test_run_backtest_win_loss_ratio_positive_formula` tests the formula directly (not via full backtest run) â€” simpler and equivalent for pure arithmetic
- All R2 tests use `train.WALK_FORWARD_WINDOWS` dynamically; no hardcoded fold counts
- R7 (sector concentration guard) not implemented â€” marked optional/low priority in PRD
...
### Core Validation
R9: `screen_day()` now returns `None` for fallback-stop entries (no structural pivot support); `stop_type` is always `'pivot'` for returning signals. R8: earnings-proximity guard added â€” entries within 14 calendar days of next earnings are rejected; backward-compatible with old parquet files lacking `next_earnings_date`. R10: `manage_position()` forces exit at `max(current_stop, price_10am)` for positions held >30 business days with unrealised PnL < 30% of RISK_PER_TRADE. `prepare.py` extended with `_add_earnings_dates()` helper. `program.md` updated: FOLD_TEST_DAYS default 20â†’40 (R1), consistency floor auto-calibrated to `âˆ’RISKÃ—MAX_SIMULTANEOUS_POSITIONSÃ—10` (R3), deadlock detection pivot paragraph (R5), position-management priority in iterations 6â€“10 (R6).

### Test Status
- Automated: âœ… 64 passed (+18 new V4-A unit tests), 0 failures, 0 new regressions
...
### Notes
- Existing parquet files lack `next_earnings_date` column â€” delete cache and re-run `prepare.py` before next optimization session for R8 to take effect
- Volume threshold co-changed from â‰¥1.0Ã— to â‰¥1.9Ã— MA30 alongside R9; fixtures updated accordingly
- GOLDEN_HASH not modified â€” immutable zone untouched
...
### Core Validation
SESSION SETUP / STRATEGY TUNING comment headers added to `train.py` mutable section; `RISK_PER_TRADE` comment updated with "DO NOT raise to inflate P&L." `program.md` updated with five targeted edits: SESSION SETUP scope instruction, `TICKER_HOLDOUT_FRAC = 0.1` recommended default, dual keep/discard condition (`min_test_pnl` + `train_pnl_consistency` floor), zero-trade plateau early-stop rule (3-iteration direction reversal; 10-iteration `plateau` status + revert), and `discard-inconsistent` status added to column 10 definition. No GOLDEN_HASH update required â€” immutable zone untouched.

### Test Status
- Automated: âœ… 49 passed (+10 new V3-G unit tests), 1 pre-existing skip (git state), 0 new regressions
- Manual: none required

### Notes
- All 10 V3-G tests are text/import-level assertions â€” no live parquet cache required
- Baseline before V3-G: 39 passed, 1 skipped
- Box-drawing characters (U+2550 `â•â•`) in headers verified to survive save/re-read without corruption
...
### Test Status
- Automated: âœ… 152 passed (+10 new V3-F unit tests), 1 pre-existing skip (git state), 15 pre-existing failures unchanged
...
- `TEST_EXTRA_TICKERS = []` and env-var fallback preserve all existing behavior â€” no migration required
...
### Test Status
- Automated: âœ… 105 passed, 3 pre-existing failures (unchanged), 1 pre-existing skip
- Manual: Scenarios A/B/C require live parquet cache â€” accepted non-blocking gaps per plan
...
- `detect_regime()` cached per trading day (hoisted before screening loop) â€” all entries on the same day share the same regime value
- Code review fixes applied: bootstrap CI label mismatch (ci 0.95â†’0.90), program.md stop_type values corrected ('pivot'/'fallback'/'unknown'), look-ahead docstring note added
...
### Core Validation
R8 position cap (`MAX_SIMULTANEOUS_POSITIONS`) and correlation penalty (`CORRELATION_PENALTY_WEIGHT`) added to `run_backtest()`. R9 perturbation loop runs up to 4 jitter seeds (Â±0.5% price Ã— Â±0.3 ATR stop) and exposes `pnl_min` in return dict, `print_results()`, and `trades.tsv` annotation. End-to-end validated via live subprocess test (`test_live_train_py_subprocess_outputs_pnl_min`) that runs `train.py` with real cached data and asserts all fold `pnl_min` lines are present, parseable, and â‰¤ `total_pnl`.

### Test Status
- Automated: âœ… 24 passed (+9 new V3-C unit tests, +1 live subprocess test), 1 pre-existing skip (git state)
- Manual: none required
...
### Test Status
- Automated: âœ… 14 passed (+9 new V3-B unit tests, +2 live-cache tests), 1 pre-existing skip (git state)
- Manual: none required
...
- NaN guard added to `portfolio_value` computation â€” all 17 cached tickers have one `price_10am = NaN` row (2026-02-02 data gap); without guard, `max_drawdown` returned NaN
...
### Test Status
- Automated: âœ… 73 passed (+7 new V3-A tests), 9 pre-existing failures (test_registry.py ImportError, unrelated), 1 skipped
- Manual: none required
...
- Code review flagged: `test_run_backtest_risk_proportional_sizing` verifies the math identity, not the actual sizing applied by `run_backtest()` â€” minor coverage gap, non-blocking
...
### Core Validation
`strategies/` package created with REGISTRY, `base_indicators.py`, and `energy_momentum_v1.py` (extracted from `e9886df`). `strategy_selector.py` calls `claude -p` CLI (no API key â€” runs inside Claude Code); `_call_claude()` strips `CLAUDECODE` env var before spawning subprocess. Integration test `test_select_strategy_real_claude_code` makes a real CLI call for XOM and validates response shape and strategy validity.

### Test Status
- Automated: âœ… 120 passed, 1 skipped (unit + integration + e2e, full suite via `.venv`)
- Manual: none required

### Notes
- Selector uses `claude -p <prompt>` subprocess â€” requires Claude Code on PATH; not portable outside Claude Code
...
## Feature: Optimization Harness Overhaul (Enhancements 1â€“5)
...
### Test Status
- Automated: âœ… 58/61 passing (3 pre-existing failures unrelated to this feature)
- Manual: none required

---

## Run: mar20 â€” Energy/Materials Universe (2026-03-20)
...
|--------|--------|-----------|--------------|----------|--------|-------------|
| 48191e7 | 0.000 | $0 | 0 | â€” | keep | baseline (strict pullback screener, no trades) |
| 2ceed17 | 2.809 | â€” | 1 | â€” | keep | NaN guard + CCI -30 + pullback 5% |
| 68ac7ae | 5.176 | â€” | 18 | â€” | keep | momentum breakout: SMA50 + 20d high + vol 1.2Ã— |
| b95992a | 5.266 | â€” | 19 | â€” | keep | relax volume to 1.0Ã— MA30 |
| 2a3c800 | 5.382 | â€” | 19 | â€” | keep | add price_10am > prev day high |
| **e9886df** | **5.791** | **$952.88** | **18** | **77.8%** | **keep** | **add RSI(14) 50â€“75 filter** |

Avg PnL/trade: **$52.94** | Backtest: 2025-12-20 â†’ 2026-03-20 | Universe: CTVA, LIN, XOM, DBA, SM, IYE, EOG, APA, EQT, CTRA, APD, DVN, BKR, COP, VLO, HEI, HAL

### Multi-Sector Optimization Results (2026-03-20)

| Sector / Run | Branch | Window | Best Sharpe | Trades | Total PnL | Tag |
|---|---|---|---|---|---|---|
| Energy (in-sample) | `autoresearch/mar20` | Dec 20 â†’ Mar 20, 2026 | 5.791 | 18 | â€” | `energy-momentum-v1` |
| Energy (OOS validation) | `autoresearch/energy-oos-sep25` | Sep 20 â†’ Dec 20, 2025 | -0.010 | 6 | -$85 | â€” |
| Energy (OOS optimized) | `autoresearch/energy-oos-opt-sep25` | Sep 20 â†’ Dec 20, 2025 | 8.208 | 73 | ~$956 | `energy-oos-v1` |
| Semis | `autoresearch/semis-mar20` | Dec 20 â†’ Mar 20, 2026 | 4.754 | 34 | $602 | `semis-momentum-v1` |
| Utilities | `autoresearch/utilities-mar20` | Dec 20 â†’ Mar 20, 2026 | 4.363 | 29 | $47 | `utilities-breakout-v1` |
| Financials | `autoresearch/financials-mar20` | Dec 20 â†’ Mar 20, 2026 | 6.575 | 45 | **-$60** | `financials-v1` |

**Key finding â€” Sharpe metric flaw:** Financials produced Sharpe 6.58 with -$60 PnL. `daily_values` is raw mark-to-market sum (not capital returns); on days with no positions it is $0, so any change that holds positions longer reduces stop-management variance â†’ artificially high Sharpe. This is why Sharpe was replaced with `train_total_pnl` as the optimization criterion (Enhancement 2).

**OOS validation result: FAIL (Sharpe -0.01)** â€” the in-sample energy strategy overfit to Decâ€“Mar 2026 regime.

**Detailed analysis**: `.agents/progress-archive/mar20-analysis-and-next-steps.md`

---

## Feature: Phase 5 â€” End-to-End Integration Test + Post-Phase Enhancements
...
### Core Validation
Full pipeline validated: `prepare.py` â†’ parquet cache â†’ `run_backtest()` â†’ output block. 9 integration tests cover schema, Sharpe consistency, output format, and multi-iteration loop simulation.

### Test Status
- Automated: âœ… 86/86 passing (9 new integration + 77 pre-existing)
- Manual: none required

### Notes
- `price_10am` always-NaN bug fixed: yfinance 1h bars label the 9:30 AM bar at 9:30 ET, not 10:00 AM
- Windows cp1252 encoding fix applied to `prepare.py` (`â†’` â†’ `->` in print statements)
- `manage_position()` breakeven trigger raised from 1Ã—ATR to 1.5Ã—ATR (matches `screen_day()` entry guard)
...
## Feature: Phase 4 â€” Agent Instructions (`program.md`)
...
### Test Status
- Automated: âœ… 74/74 passing (23 new + 51 pre-existing)
- Manual: none required

---

## Feature: Phase 3 â€” Strategy + Backtester (`train.py`)
...
### Test Status
- Automated: âœ… 51/51 passing (15 new + 36 pre-existing)
- Manual: none required

---

## Feature: Phase 2 â€” Data Layer (`prepare.py`)
...
### Test Status
- Automated: âœ… 16/16 passing (14 mock + 1 integration + 1 subprocess)
- Manual: none required

---

## Feature: Feature 2 â€” Screener (`screen_day`)
...
### Test Status
- Automated: âœ… 20/20 passing (19 original + 1 added in code review)
- Manual: none required
...
1. `uv run prepare.py` â€” downloads and caches OHLCV data for the configured tickers
2. `uv run train.py` â€” runs the backtest, prints a fixed-format results block
3. Agent loop (via `program.md`) â€” autonomously mutates `train.py`, commits, backtests, keeps or reverts
...
## Feature: Strategy Refactor â€” Stateful Scanner, Perf, and Cleanup

**Status**: âœ… Complete (Phases 1â€“5; Phase 0 and RC1â€“RC6 deferred)
**Plan File**: `.agents/plans/strategy_refactor.md`

### Reports Generated

**Execution Report:** `.agents/execution-reports/strategy_refactor.md`
- Phases 1â€“5 complete; `process_scan_bar` is the single source of truth for all scanning logic
...
- Manual validation (backtest_smt.py output vs pre-refactor baseline) deferred â€” requires Databento parquets
...
### Baseline (post-Plans-1-2-3 defaults, full 6-fold run 2024-09-01 â†’ 2026-03-20)
...
- **Walk-forward**: 6 folds Ã— 60 business-day test windows
...
#### 1. Asymmetric stop ratios â€” HIGHEST PRIORITY
`LONG_STOP_RATIO` and `SHORT_STOP_RATIO` are both `0.45` (RR â‰ˆ 2.22 for both sides).
...
- Suggested search space: `LONG_STOP_RATIO` âˆˆ [0.30, 0.55], `SHORT_STOP_RATIO` âˆˆ [0.30, 0.55]
- Optimise for: `mean_test_pnl` (primary), `min_test_pnl` > 0 (guard)

#### 2. Session window â€” HIGH PRIORITY
Kill zone is 9:00â€“10:30. Pre-cash (9:00â€“9:30) and RTH open (9:30â€“10:30) behave differently.
...
- Watch: narrowing window reduces trade count â€” need â‰¥80 test trades total to stay meaningful

#### 3. Minimum divergence magnitude â€” MEDIUM PRIORITY
...
- Add a `MIN_DIVERGENCE_POINTS` constant (e.g., 2â€“8 MNQ points) â€” currently absent.
- Expected effect: fewer signals, lower stop-out rate, higher avg_rr.

#### 4. MIN_BARS_BEFORE_SIGNAL â€” MEDIUM PRIORITY
...
- Suggested search space: 2â€“8 bars (10â€“40 min at 5m)
- Trade-off: more bars = fewer signals but stronger context.

#### 5. Entry confirmation tightening â€” LOWER PRIORITY
...
- Add `ENTRY_CLOSE_STRENGTH` constant âˆˆ [0.0, 0.5] (0.0 = current behaviour, disabled)

#### 6. TDO definition variant â€” LOWER PRIORITY
...
Primary: **maximise `mean_test_pnl`** (average fold P&L) â€” maximises total return across regimes.
Secondary: **`min_test_pnl` > 0** (all qualified folds profitable), **Sharpe â‰¥ 2.0 in every fold**, total test trades â‰¥ 80.
