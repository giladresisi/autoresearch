...
@pytest.fixture
def arm2_on(monkeypatch):
    monkeypatch.setattr("agent.stretch_override.HALTED_ARM_ENABLED", True)


def test_2_off_a_stale_stretch_falls_through_however_unretraced():
    """Arm 2 is OFF since 2026-09-24 (its fire days continued 66%, n=41). A stretch older
    than the clock does NOT fire, whatever the retrace -- the model decides, nothing is
    forced either way."""
    for age in (AGE_MAX_MIN + 1, 46.0, 150.0):
        for retrace in (0.0, 5.0, RETRACE_MAX_PCT, 50.0):
            v = stretch_override(_st("UP", age=age, retrace=retrace))
            assert v["fires"] is False and v["direction"] is None, f"age={age} retrace={retrace}"
            assert "arm 2 disabled" in v["reason"]
    # arm 1 is untouched
    assert stretch_override(_st("UP", age=AGE_MAX_MIN, retrace=40.0))["fires"] is True


def test_2_a_halted_but_unretraced_stretch_fires_however_old_it_is(arm2_on):
    """Arm 2 WHEN ENABLED, restored 2026-09-17 by user decision AGAINST the D0 measurement (the arm
...
def test_3_a_halted_and_retraced_stretch_does_not_fire(arm2_on):
...
def test_3b_the_two_arms_are_independent(arm2_on):
...
@pytest.mark.timeout(300)
def test_8_the_real_2026_09_17_stretch_is_arm_2_only(monkeypatch):
    """MNQ: UP 505.75 pts from 29247.50 (18:04 the evening before) to 29753.25 at 08:34:34 —
    46 minutes old at 09:20, 5.0% retraced. Arm 2 off (default): falls through to the model.
    Arm 2 on: forces DOWN, as it did 2026-09-17..09-24."""
