"""
strategies/financials_mar20.py — Extracted from autoresearch/financials-mar20 @ b4eb815
"""

METADATA = {
    "name":         'financials-mar20',
    "sector":       'financials',
    ...
    "test_end":     '2026-03-20',
    "source_branch": 'autoresearch/financials-mar20',
    "source_commit": 'b4eb815',
    "train_pnl":    None,
    "train_sharpe": 6.575348,
    "train_trades": 45,
    "description":  (
        "This momentum breakout strategy enters long when the 10am price clears both the 20-day closing high and the prior day's high, with RSI(14) in the 65â€“90 zone and price above the 50-period SMA, filtering out setups where the last three bars show tight high clustering (stalling at ceiling). The initial stop is set 0.3Ã—ATR14 below the nearest validated pivot low (requiring at least one prior support touch within 90 bars, no more than 10 zone touches, and a minimum 1.5Ã—ATR gap to entry), falling back to a hard 1.5Ã—ATR stop if no qualifying pivot exists; the stop is raised to breakeven only after price extends 5Ã—ATR above entry. This strategy suits a trending bull market with strong sector momentum, where breakouts from consolidation ranges are likely to follow through rather than immediately revert."
    ),
}
...
def screen_day(df: pd.DataFrame, today) -> "dict | None":
    ...
    # Rule 3: compute vol_ratio for signal metadata (no filtering)
    vol_ratio = float(df['volume'].iloc[-1]) / vm30

    # Rule 3b: RSI between 65 and 90 (very high momentum zone)
    if not (65 <= rsi <= 90):
        return None
    ...
    # Stop: prefer pivot-low stop, fall back to 1.5 ATR
    stop = find_stop_price(df, price_10am, atr)
    if stop is None:
        stop = round(price_10am - 1.5 * atr, 2)

    # 1.0 ATR buffer safety net (looser, more trades through)
    if price_10am - stop < 1.0 * atr:
        return None
